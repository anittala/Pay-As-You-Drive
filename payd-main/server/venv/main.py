...
@app.route('/tripOver/<userId>/<tripId>', methods=["GET"])
def fetch_firebase(userId, tripId):
    now = datetime.now()
    ...
    for i in range(len(SPEED)):
        ...
        if SPEED[i] > SPEEDLIMIT[i]:
            count += 1
    speeding_behaviour = count/len(SPEED)

    aggressive_behaviour = (1*(speeding_behaviour)+2*(event_behaviour))*100/3
    ...
    analysis = {'sudden_accelerations': behaviour[0], 'sudden_right_turns': behaviour[1],
                ...
                'aggresive_percentage': aggressive_behaviour, 'speed_exceeds': count, 'time_stamp': dt_string}
